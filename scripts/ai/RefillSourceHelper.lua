...
function RefillSourceHelper:checkSource(myVehicle, fieldPolygon, source, fillUnitIndex)
    ...
    local isAdUnloading = rootVehicle and rootVehicle.ad and rootVehicle.ad.trailerModule and rootVehicle.ad.trailerModule.isUnloading
    local isAdEnRoute = rootVehicle and rootVehicle.ad and rootVehicle.ad.stateModule
            and rootVehicle.ad.stateModule:isActive() and not rootVehicle.ad.drivePathModule:isTargetReached()
    CpUtil.debugVehicle(self.debugChannel, myVehicle,
            'Fill source candidate %s (fill unit %s): on field %s, closest distance %.1f, root vehicle %s, last speed %.1f, CP active %s, AD target %s, AD-unload %s, fill level %.1f/%.1f',
            CpUtil.getName(source), fillUnitIndex, isOnField and '' or 'NOT', closestDistance,
            rootVehicle and CpUtil.getName(rootVehicle) or 'none', lastSpeed, isCpActive, isAdEnRoute and 'yes' or 'no', isAdUnloading and 'yes' or 'no', fillLevel, capacity)
    if isOnField and rootVehicle ~= myVehicle and not isCpActive and lastSpeed < 0.1 and hasFluid and (not isAdEnRoute or isAdUnloading) then
